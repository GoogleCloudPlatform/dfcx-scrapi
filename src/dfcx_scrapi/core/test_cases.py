...
class TestCases(ScrapiBase):
    ...
    def get_covered_intents(
        self,
        agent_id: str = None,
        ):
        """
        Get the intent wise test cases coverage information.

        Args:
          agent_id: The agent to get coverage from. Format:
          `projects/<Project ID>/locations/<Location ID>/agents/<Agent ID>`.
        Returns:
          response: Dictionary with intents as key and coverage info as value.
        """
        ...
        intents_msg = response.intent_coverage.intents

        for msg in intents_msg:
            intents_covered[msg.intent] = msg.covered

        return intents_covered
    ...
    def get_covered_pages(
        ...
        """
        Get the page wise test cases coverage information.

        Args:
            agent_id: The agent to get coverage from. Format:
            `projects/<Project ID>/locations/<Location ID>/agents/<Agent ID>`.
        Returns:
            response: List of tuples having infomration of source page,
            target page and coverage.
        """
        ...
        response =  client.calculate_coverage(request = request)
        transitions_covered = response.transition_coverage.transitions

        for transition in transitions_covered:
            source_page = transition.source.page.display_name
            target_page = transition.target.page.display_name
            pages_covered.append((source_page, target_page, transition.covered))

        return pages_covered

    def get_covered_flows(
        self,
        agent_id: str = None,
        ):
        """
        Get the flow wise test cases coverage information.

        Args:
            agent_id: The agent to get coverage from. Format:
            `projects/<Project ID>/locations/<Location ID>/agents/<Agent ID>`.
        Returns:
            response: List of tuples having infomration of source flow,
            target flow and coverage.
        """
        ...
        for transition in transitions_covered:
            source_flow = transition.source.flow.display_name
            target_flow = transition.target.flow.display_name
            flows_covered.append((source_flow, target_flow, transition.covered))

        return flows_covered

    def get_covered_route_groups(
        self,
        agent_id: str = None,
        ):
        """
        Get the route group wise test cases coverage information.

        Args:
            agent_id: The agent to get coverage from. Format:
            `projects/<Project ID>/locations/<Location ID>/agents/<Agent ID>`.
        Returns:
            response: Dictionary with route groups as key and
            coverage score as value.
        """
        ...
        response =  client.calculate_coverage(request = request)
        routes_covered = response.route_group_coverage.coverages
        for route in routes_covered:
            route_group_covered[route.route_group.display_name] = route.coverage_score

...
class NaturalLanguageUnderstandingUtil(scrapi_base.ScrapiBase):
  ...
  def __init__(self, agent_id: str, flow_display_name: str,
               ...
               creds_dict: Dict[str,str] = None, creds=None):
    super().__init__(
  ...
  def _load_data(self, agent_id: str, flow_display_name: str,
                 page_display_name: str):
    flow_loader = flows.Flows(creds=self.creds)
    self.flow = flow_loader.get_flow_by_display_name(flow_display_name,
                                                     agent_id)
    ...
    if page_id is None:
      raise ValueError(f"Page \"f{page_display_name}\" does not exist in the "
                       "specified agent.")
  ...
  def _build_searcher(self, embeddings, num_neighbors=10):
    normalized_dataset = embeddings / np.linalg.norm(embeddings,
                                                     axis=1)[:, np.newaxis]

    # Use ScaNN brute force. This is fine for up to ~20k points
    searcher = (
        scann.scann_ops_pybind
        .builder(normalized_dataset, num_neighbors, "dot_product")
        .score_brute_force()
        .build()
    )
  ...
  def find_new_groups(self, utterances):
    ...
    # Count how many new utterances are more similar than any training phrase.

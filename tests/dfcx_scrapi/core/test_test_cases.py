        ...
        test_case_conversation_turns=[
            ...
            types.ConversationTurn(
                virtual_agent_output=types.ConversationTurn.VirtualAgentOutput(
                    ...
                    text_responses=[types.ResponseMessage.Text(
...
def mock_list_tc_pager_no_turns(mock_tc_obj_no_turns):
    return test_cases.pagers.ListTestCasesPager(
        test_cases.TestCasesClient.list_test_cases,
        types.test_case.ListTestCasesRequest(),
        types.test_case.ListTestCasesResponse(test_cases=[mock_tc_obj_no_turns])
        )
...
def test_get_flow_id_from_test_config(mock_tc_obj_turns, test_config):
    ...
    tc = TestCases()
    for str_value, id in tests:
        if str_value == "flow":
            mock_tc_obj_turns.test_config.flow = id
        elif str_value == "page":
            mock_tc_obj_turns.test_config.page = id
        else:
...
def test_get_page_display_name_flow_exists(test_config):
    ...
    res = tc._get_page_display_name(
        test_config["flow_id"], test_config["page_id"],
...
def test_get_page_display_name_flow_exists_no_page_map(test_config):
    ...
    res = tc._get_page_display_name(test_config["other_flow_id"], None,
                                    test_config["pages_map"])
    assert res is None
...
def test_get_page_display_name_flow_does_not_exist(test_config):
    ...
    res = tc._get_page_display_name(
        f"{test_config['agent_id']}/flows/doesnt-exist-1234", None,
        test_config["pages_map"])
    assert isinstance(res, str)
    assert res == "START_PAGE"

def test_process_test_case(mock_tc_obj_turns, test_config):
    cols = ['display_name', 'id', 'short_id', 'tags', 'creation_time',
            'start_flow', 'start_page', 'passed', 'test_time']
    tc = TestCases()

    res = tc._process_test_case(mock_tc_obj_turns, test_config["flows_map"],
                                test_config["pages_map"])
...
def test_list_test_cases_agent_id_in_method(
    ...
    mock_client.return_value.list_test_cases.return_value = mock_list_tc_pager_no_turns
...
@patch('dfcx_scrapi.core.test_cases.services.test_cases.TestCasesClient')
def test_update_test_case_with_id_and_kwargs(mock_client, test_config,
                                             mock_tc_obj_turns):
...
@patch('dfcx_scrapi.core.test_cases.services.test_cases.TestCasesClient')
def test_update_test_case_with_obj_id_and_kwargs(mock_client, test_config,
                                                 mock_tc_obj_turns):
    ...
    result = tc.update_test_case(
        test_case_id=test_config["test_case_id"], obj=mock_tc_obj_turns,
        display_name="mock test case object update")

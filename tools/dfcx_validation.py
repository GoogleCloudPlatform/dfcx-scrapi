...
class ValidationKit:
    ...
    def intent_disambg(self, agent_id, refresh=False, flow=None):
        ...
        for index, row in validation_df.iterrows():
            ...
            if bool(
                re.search(
                    ...
                    deets)):
                intents = re.findall(
                    'Intent \'(.*)\': training phrase ', deets)
                training_phrases = re.findall(
                    'training phrase \'(.*)\'', deets)
        ...
        intent_options = extraction.groupby(['disambig_id'])['intent'].apply(
            list).reset_index().rename(columns={'intent': 'intents'})
        ...
        external = extraction.groupby(
                ...
                'disambig_id']).agg(
            {
                ...
                'intents': 'first',
                'intent': 'count'}).reset_index().rename(
                    columns={
                        'intent': 'conflicting_tp_count'})

...
class CopyUtil(SapiBase):
        ...
        try:
            if copy_option == "create":
                ...
                self.intents.update_intent(
                    destination_intent_obj.name, intent_object
                )
                logging.info(
    ...
    def create_page_shells(
        ...
        for page in pages_list:
            try:
                ...
                logging.info(
                    "Page '{}' created successfully".format(page.display_name)
                )
            except Exception as e:
                logging.info(e)
                logging.info(
    ...
    def copy_paste_agent_resources(
        self,
        ...
        if "webhooks" in resource_obj_dict and "webhooks" not in skip_list:
            ...
            for webhook in resource_obj_dict["webhooks"]:
                logging.info(
                    "Creating Webhook '{}'...".format(webhook.display_name)
                )
                try:
        ...
        if "entities" in resource_obj_dict and "entities" not in skip_list:
            for entity in resource_obj_dict["entities"]:
                logging.info(
                    "Creating Entity '{}'...".format(entity.display_name)
                )
        ...
        if "intents" in resource_obj_dict and "intents" not in skip_list:
            ...
            for intent in resource_obj_dict["intents"]:
                logging.info(
                    "Creating Intent '{}'...".format(intent.display_name)
                )
                time.sleep(1)
                ...
                if "parameters" in intent:
                    for param in intent.parameters:
                        if "sys." in param.entity_type:
                            ...
                            source_name = source_entities_map[param.entity_type]
                            destination_name = destination_entities_map[
                                source_name
                            ]
                            param.entity_type = destination_name
        ...
        if (
            "route_groups" in resource_obj_dict
            ...
            for rg in resource_obj_dict["route_groups"]:
                logging.info(
                    "Creating Route Group '{}'...".format(rg.display_name)
                )
            ...
            for page in pages_mod:
                if "entry_fulfillment" in page:
                    if "webhook" in page.entry_fulfillment:
                        page.entry_fulfillment.webhook = webhooks_map[
                            page.entry_fulfillment.webhook
                        ]
                    ...
                    for tr in page.transition_routes:
                        if "target_page" in tr:
                            if tr.target_page.split("/")[-1] == "END_FLOW":
                                ...
                                tr.target_page.split("/")[-1] == "CURRENT_PAGE"
                            ):
                        ...
                        if "intent" in tr:
                            ...
                            if "webhook" in tr.trigger_fulfillment:
                                tr.trigger_fulfillment.webhook = webhooks_map[
                                    tr.trigger_fulfillment.webhook
                                ]
                        elif (
                            "condition" in tr
                            and "webhook" in tr.trigger_fulfillment
                        ):
                            tr.trigger_fulfillment.webhook = webhooks_map[
                                tr.trigger_fulfillment.webhook
                            ]
                        ...
                        for param in page.form.parameters:
                            if "fill_behavior" in param:
                                if (
                                    ...
                                    in param.fill_behavior
                                ):
                                    if (
                                        "webhook"
                                        in param.fill_behavior.initial_prompt_fulfillment
                                    ):
                                ...
                                if (
                                    ...
                                    for (
                                        ...
                                        param.fill_behavior.reprompt_event_handlers
                                    ):
                                        if "trigger_fulfillment" in handler:
                                            if (
                                                "webhook"
                                                in handler.trigger_fulfillment
                                            ):
                            ...
                            if "sys." in param.entity_type:
                                ...
                                param.entity_type = entities_map[
                                    param.entity_type
                                ]
    ...
    def get_page_dependencies(self, obj_list):
        ...
        for page in obj_list:
            if (
                "entry_fulfillment" in page
                and "webhook" in page.entry_fulfillment
            ):
